...
<title>Search Booking | Doc UiTM</title>
	<link rel="stylesheet" type="text/css" href="css/meslist.css">
...
<body background="image/medBG-green.png">
	...
	<section class="banner" >
		...
		<div class="title-highlight"> <br><br><br><br><br><br><br><br><br><br><br>
			...
			<h1><mark><a> List of Bookings </a></mark></h1><br>
			<h2> Click on the booking ID to view each bookings</h2>
		</div>
	</section>
	...
	<div class="row">
		...
		<div class="col-md-4">
			<form action="SearchBookingController" method="get">
				<input type="text" class="form-control" name="q" placeholder="Search">
			</form>
	</div>
		<div class="col-md-4 text-right" style="padding-right:150px; padding-bottom:30px">
			<a href="BookingController?action=listBookings&role=<%= role %>" class="btn btn-primary">Show All</a><br><br>
		</div>
	</div>
	<div class="table-pad">
	<table class="table table-bordered table-stripped table-hover">
		...
		<tbody>
		<c:forEach items="${bookings}" var="book" varStatus="bookings">
		<tr>
			<td><a href="BookingController?action=viewBookings&role=<%= role %>&bookingid=<c:out value="${book.bookingid}"/>"><c:out value="${book.bookingid}"/></a></td>
			<td><c:out value="${book.phonenumber}"/></td>
			<td><c:out value="${book.purpose}"/></td>
			<td><c:out value="${book.bookingdate}"/></td>
			<td><c:out value="${book.bookingtime}"/></td>
			<td><a href="BookingController?action=updateBookings&role=<%= role %>&bookingid=<c:out value="${book.bookingid}"/>"class="lol">Update</a>
			<a href="BookingController?action=deleteBookings&role=<%= role %>&bookingid=<c:out value="${book.bookingid}"/>"class="lol">Cancel</a></td>	
		</tr>
		</c:forEach>
 		</tbody>
	</table>
